...
// PingOtherPeer pings other peer to check if they are available
func (ipfs *Connector) PingOtherPeer(ctx context.Context, p api.IPFSID) (string, error) {
	ctx, span := trace.StartSpan(ctx, "ipfsconn/ipfshttp/pingOtherPeer")
	defer span.End()

	ctx, cancel := context.WithTimeout(ctx, ipfs.config.IPFSRequestTimeout)
	defer cancel()
	url := "ping?arg=" + p.ID.String()
	str, errr := ipfs.postCtxNew(ctx, url, "", nil)
	fmt.Printf("PPPPPPPPPPIIIIIIIIINNNNNNNNNGGGGGGGGGGGGGGGG : %s \n", str)
	return str, errr
}
...
func (ipfs *Connector) NodeGet(ctx context.Context, c cid.Cid) (string, error) {
	ctx, span := trace.StartSpan(ctx, "ipfsconn/ipfshttp/BlockGet")
	defer span.End()

	ctx, cancel := context.WithTimeout(ctx, ipfs.config.IPFSRequestTimeout)
	defer cancel()
	url := "dag/get?arg=" + c.String()
	return ipfs.postCtxNew(ctx, url, "", nil)
}
...
// the ipfs daemon, reads the full body of the response and
// returns it after checking for errors.
func (ipfs *Connector) postCtxNewch(ctx context.Context, path string, contentType string, postBody io.Reader) ([]byte, error) {
	rdr, err := ipfs.postCtxStreamResponse(ctx, path, contentType, postBody)
	if err != nil {
		return nil, err
	}
	...
	fmt.Fprintf(os.Stdout, "Data length : %d\n", len(body))
	// Deserialize the response into a DAG node
	/*var node ipld.Node
	err = json.Unmarshal(body, &node)
	if err != nil {
		log.Fatalf("Failed to unmarshal response into DAG node: %v", err)
	}*/
	return body, nil
}
...
func prettyPrintJSON(body []byte) (string, error) {
	...
	err := json.Indent(&prettyJSON, body, "", "  ") // Format JSON with indentation
	if err != nil {
		return "", err
	}
