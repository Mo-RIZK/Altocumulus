...
func (dc distanceChecker) isClosestNeww(ci api.Cid, repairing []peer.ID) (bool, []peer.ID) {
	if len(repairing) == len(dc.otherPeers)+1 {
		repairing = make([]peer.ID, 0)
	}
	repair := true
	tocheck := removeElements(append(dc.otherPeers, dc.local), repairing)
	if slices.Contains(repairing, dc.local) {
		repair = false
		peer := dc.Whoisresponsible(ci, tocheck)
		repairing = append(repairing, peer)
		return repair, repairing
	} else {
		var peer peer.ID
		peer, repair = dc.WhoisresponsibleWithMe(ci, tocheck)
		repairing = append(repairing, peer)
		return repair, repairing
	}
}

func (dc distanceChecker) WhoisresponsibleWithMe(ci api.Cid, tocheck []peer.ID) (peer.ID, bool) {
	var distresponsible distance
	var peerresponsible peer.ID
	ciHash := convertKey(ci.KeyString())
	localPeerHash := dc.convertPeerID(dc.local)
	distresponsible = xor(ciHash, localPeerHash)
	peerresponsible = dc.local
	...
	if peerresponsible == dc.local {
		return peerresponsible, true
	}
	return peerresponsible, false
}

func (dc distanceChecker) Whoisresponsible(ci api.Cid, tocheck []peer.ID) peer.ID {
	var distresponsible distance
	var peerresponsible peer.ID
	ciHash := convertKey(ci.KeyString())
	localPeerHash := dc.convertPeerID(tocheck[0])
	distresponsible = xor(ciHash, localPeerHash)
	peerresponsible = tocheck[0]
	for _, p := range tocheck {
		peerHash := dc.convertPeerID(p)
		distance := xor(peerHash, ciHash)
		// if myDistance is larger than for other peers...
	...
	return peerresponsible
}

// removeElements removes all elements in removeFrom from the original slice
func removeElements(original []peer.ID, toremove []peer.ID) []peer.ID {
	// Create a map to store elements to remove for fast lookup
	removeSet := make(map[peer.ID]bool)
	for _, id := range toremove {
		removeSet[id] = true
	}

	// Filter out elements from the original slice
	var result []peer.ID
