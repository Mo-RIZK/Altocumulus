...
func New(cfg *Config, pid peer.ID, peerName string, getState func(ctx context.Context) (state.ReadOnly, error), connector *ipfshttp.Connector) *ECRepairS {
	...
	spt := &ECRepairS{
		...
		RepairCh:  make(chan *api.Pin, cfg.MaxPinQueueSize),
		connector: connector,
	}

	for i := 0; i < spt.config.ConcurrentPins; i++ {
		go spt.opWorker(spt.pin, spt.RepairCh)
	}
	return spt
}
...
// and unpinning.
func (spt *ECRepairS) opWorker(pinF func(pin *api.Pin) error, RepairCh chan *api.Pin) {
	...
	APPLY_OP:
		applyPinF(spt.pin, op)
	}
}

// applyPinF returns true if the operation can be considered "DONE".
func applyPinF(pinF func(pin *api.Pin) error, op *api.Pin) bool {
	pinF(op)    // call pin/unpin
...
func (spt *ECRepairS) Shutdown(ctx context.Context) error {
...
func (spt *ECRepairS) unpin(op *optracker.Operation) error {
...
func (spt *ECRepairS) Track(ctx context.Context, c api.Pin) error {
...
func (spt *ECRepairS) Untrack(ctx context.Context, c api.Cid) error {
...
func (spt *ECRepairS) StatusAll(ctx context.Context, filter api.TrackerStatus, out chan<- api.PinInfo) error {
...
func (spt *ECRepairS) Status(ctx context.Context, c api.Cid) api.PinInfo {
...
func (spt *ECRepairS) RecoverAll(ctx context.Context, out chan<- api.PinInfo) error {
	defer close(out)
...
func (spt *ECRepairS) Recover(ctx context.Context, c api.Cid) (api.PinInfo, error) {
...
func (spt *ECRepairS) recoverWithPinInfo(ctx context.Context, pi api.PinInfo) (api.PinInfo, error) {

	return spt.Status(ctx, pi.Cid), nil
}

func (spt *ECRepairS) ipfsPins(ctx context.Context) (<-chan api.IPFSPinInfo, <-chan error) {
...
func (spt *ECRepairS) PinQueueSize(ctx context.Context) (int64, error) {
	return 0, nil
}
...
func (spt *ECRepairS) OpContext(ctx context.Context, c api.Cid) context.Context {
	return nil
}
